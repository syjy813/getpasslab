...
priority: 1차
status: 완료
examComment: 연결 기출 3문항은 직렬계의 신뢰도 곱셈과 병렬계의 동시고장 확률 차감을 계산함
---
...
- 신뢰도 0.6인 인간과 0.9인 기계가 직렬로 작업하면 `0.6 × 0.9 = 0.54`임
- 같은 두 요소가 독립적인 병렬 구조라면 `1-(1-0.6)(1-0.9)=0.96`임
- 타이어 하나가 파열되지 않을 확률이 0.99이고 네 타이어가 모두 정상이어야 한다면 자동차 시스템의 신뢰도는 `0.99⁴ ≈ 0.961`임
...
|---|---|
| Rseries | **$R_{series}=\prod_{i=1}^{n}R_i$** |
| Rparallel | **$R_{parallel}=1-\prod_{i=1}^{n}(1-R_i)$** |
| n개 병렬 | **$1-(1-r)^n$** |
