...
<strong>가용도(Availability)</strong>는 장비가 전체 관측 시간 중 정상적으로 작동할 수 있는 비율이다. 수리가 가능한 장비에서는 평균고장간격(MTBF)과 평균수리시간(MTTR)의 관계로 표현한다.
...
2019년 3월 시험 36번은 가용도와 MTTR로 MTBF를 구하는 문제다. 보기의 18시간은 위 식을 변형한 결과이며, 단위는 시간으로 유지한다.
