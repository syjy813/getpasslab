---
title: 안전표지 종류
slug: safety-signs
subject_id: 1
group: 안전관리 기준·표지
tags: [개념]
summary: 금지·경고·지시·안내 4종
questions: [20220424_014, 20220305_014, 20210814_005, 20210307_016, 20210515_003, 20210515_012, 20200606_001, 20200822_013, 20200822_012, 20200926_009, 20180304_015, 20180304_017, 20180428_007, 20180819_011, 20190303_017]
order: 1
priority: 1차
status: 완료
examComment: 표지 유형·색채 의미·기본 모형을 적용 상황과 함께 구분하는 문제가 반복됨
---
## 핵심 개념

### 표지의 4가지 유형

안전·보건표지는 전달하려는 행동과 위험의 성격에 따라 금지·경고·지시·안내로 구분함

<figure class="chapter-figure chapter-figure-document">
  <div class="chapter-figure-page chapter-figure-page-1">
    <img
      src="/images/chapters/safety-signs/industrial-safety-health-rules-annex-6-page-1.gif"
      alt="산업안전보건법 시행규칙 별표 6 안전보건표지 원본 1쪽: 금지표지, 경고표지, 지시표지"
      width="651"
      height="911"
      loading="lazy"
      decoding="async"
    />
    <span class="chapter-figure-section-title chapter-figure-section-title-prohibition" aria-hidden="true"><small>1.</small><strong>금지표지</strong></span>
    <span class="chapter-figure-section-title chapter-figure-section-title-warning" aria-hidden="true"><small>2.</small><strong>경고표지</strong></span>
    <span class="chapter-figure-section-title chapter-figure-section-title-mandatory" aria-hidden="true"><small>3.</small><strong>지시표지</strong></span>
  </div>
  <div class="chapter-figure-page chapter-figure-page-2">
    <img
      src="/images/chapters/safety-signs/industrial-safety-health-rules-annex-6-page-2.gif"
      alt="산업안전보건법 시행규칙 별표 6 안전보건표지 원본 2쪽: 안내표지, 관계자외 출입금지, 문자 추가 시 예시문"
      width="651"
      height="978"
      loading="lazy"
      decoding="async"
    />
    <span class="chapter-figure-section-title chapter-figure-section-title-guide" aria-hidden="true"><small>4.</small><strong>안내표지</strong></span>
    <span class="chapter-figure-section-title chapter-figure-section-title-restricted" aria-hidden="true"><small>5.</small><strong>관계자외<br />출입금지</strong></span>
    <span class="chapter-figure-section-title chapter-figure-section-title-copy" aria-hidden="true"><small>6.</small><strong>문자 추가 시<br />예시문</strong></span>
  </div>
  <figcaption>
    원본: 국가법령정보센터 <a href="https://www.law.go.kr/법령별표서식/(산업안전보건법 시행규칙,20260801,별표6)" target="_blank" rel="external noopener">「산업안전보건법 시행규칙」 별표 6(시행 2026 · 8 · 1.)</a>의 안전보건표지 도안 1·2쪽
    표지 도안은 원본을 유지하고 검정 분류 제목 영역만 가독성을 위해 덧씌웠으며, 학습에 필요한 표지 부분만 표시했음 · 용도·설치 장소·형태·색채 기준은 <a href="https://www.law.go.kr/법령별표서식/(산업안전보건법 시행규칙,20260801,별표7)" target="_blank" rel="external noopener">별표 7</a>에서 확인함
  </figcaption>
</figure>

| **유형** | 전달 목적 |
| --- | --- |
| **금지표지** | 특정 행동이나 출입을 하지 못하게 함 |
| **경고표지** | 위험물·위험장소·위험상태를 알림 |
| **지시표지** | 특정 보호구 착용 등 필요한 행동을 지시함 |
| **안내표지** | 비상구·피난소·통행 등 위치와 정보를 알림 |

2021년 3월 시험 16번은 보안경 착용처럼 특정 행동을 요구하는 표지가 지시표지임을 확인함 · 2019년 3월 시험 17번과 2021년 8월 시험 5번은 관계자 외 출입금지 표지의 적용 상황을 구분하는 문제임

## 판별 기준

기출에서는 색채의 사용 사례와 기본 모형을 연결하거나, 경고표지 중 모형이 다른 항목을 찾게 함 · 대표적으로 빨강·노랑·파랑·녹색의 색채 의미와 표지 유형을 연결하되, 선택지에 제시된 색도 기준과 모형을 그대로 대조함

| **확인 항목** | 문제 풀이 기준 |
| --- | --- |
| **색채** | 특정 행위의 지시·사실 고지, 위험 경고, 정지·금지, 피난·안내 중 무엇을 나타내는지 확인 |
| **기본 모형** | 표지의 테두리·도형·내용이 문제에서 제시한 유형과 맞는지 확인 |
| **적용 상황** | 출입금지, 보호구 착용, 폭발성·방사성·부식성 물질 경고 등 구체적 문맥을 확인 |

2022년 4월 시험 14번, 2021년 5월 시험 12번, 2018년 3월 시험 17번은 기본 모형을, 2021년 5월 시험 3번, 2020년 8월 시험 12번, 2020년 9월 시험 9번, 2018년 3월 시험 15번, 2018년 8월 시험 11번은 색채 또는 색도 기준을 물음 · 2022년 3월 시험 14번과 2018년 4월 시험 7번은 그림의 모양과 색상을 함께 판독하는 문제임

> **확정 주의**  색상 하나만 보고 표지 유형을 결정하지 않음 · 색채·기본 모형·표시 문구·적용 상황을 함께 확인함

## 시험 포인트

| 문제에서 보이는 키워드 | 정답으로 연결 |
|---|---|
| 금지표지 | **특정 행동이나 출입을 하지 못하게 함** |
| 경고표지 | **위험물·위험장소·위험상태를 알림** |
| 지시표지 | **특정 보호구 착용 등 필요한 행동을 지시함** |
| 안내표지 | **비상구·피난소·통행 등 위치와 정보를 알림** |
| 색채 | **특정 행위의 지시·사실 고지** · 위험 경고, 정지·금지, 피난·안내 중 무엇을 나타내는지 확인 |
| 기본 모형 | **테두리·도형·내용의 유형 일치 여부 확인** |

## 자주 틀리는 포인트

이 챕터는 금지·경고·지시·안내표지의 구분, 색상·형태·대표 적용 상황을 다룸 · 모든 표지 도안을 이미지 없이 복원하거나 국제표준과 현행 법령 전체를 확정하는 내용은 범위에서 제외함
