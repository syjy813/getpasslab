---
title: 제조물책임법 결함 유형
slug: product-liability-defects
subject_id: 1
order: 2
priority: 출시 필수
status: 완료
group: 법령·보상
tags: [개념, 법령]
summary: 제조상·설계상·표시상 결함 3유형
questions: []
related: [serious-accident-criteria, industrial-accident-insurance]
examComment: 현재 수록 기출(2018~2022)에는 직접 출제 이력이 없음. 제조상·설계상·표시상 결함의 정의를 구분하는 법령 개념.
---

## 핵심 개념

제조물책임(PL)법상 결함의 3유형.

| 유형 | 정의 |
|------|------|
| 제조상의 결함 | 제조·가공상의 주의의무 이행 여부와 관계없이, 제조물이 원래 의도한 설계와 다르게 제조·가공되어 안전하지 못하게 된 경우 |
| 설계상의 결함 | 합리적인 대체설계를 채용하였더라면 피해나 위험을 줄이거나 피할 수 있었음에도 이를 채용하지 않아 안전하지 못하게 된 경우 |
| 표시상의 결함 | 합리적인 설명·지시·경고 등의 표시를 하였더라면 피해나 위험을 줄이거나 피할 수 있었음에도 이를 하지 않은 경우 |

법의 결함 정의에는 위 세 유형 외에 그 밖에 통상적으로 기대할 수 있는 안전성이 결여된 경우도 포함된다.

## 결함 구분

| 용어 | 의미 | 주의점 |
|------|------|--------|
| 제조물 | 제조·가공된 동산 | 부동산·미가공 농산물 제외 |
| 무과실 책임 | 제조업자 과실 입증 불요 | 결함·손해·인과관계만 입증 |
| 대체설계 | 합리적 대안 설계 | 설계상 결함의 판단 기준 |

## 시험 포인트

- 3유형의 구조 = 제품 수명주기 — 설계(계획) → 제조(생산) → 표시(판매·사용 안내) 단계별로 결함 지점을 배정
- 피해자는 제조업자의 과실 대신 결함·손해·인과관계를 입증하는 책임 체계 — 안전을 "제품 품질"로 취급하는 법리라는 점이 안전관리론에 편입된 이유
- 현재 수록 기출에서는 직접 출제 이력이 확인되지 않는다. 세 유형의 정의가 정형화되어 있으므로 설계·제조·표시 키워드로 구분한다.

## 자주 틀리는 포인트

> ⚠️ **함정 주의**: 판별 열쇠는 "무엇을 했더라면" — 설계를 바꿨더라면(설계상), 경고했더라면(표시상), 설계대로 만들었더라면(제조상). "사용상의 결함" 같은 법에 없는 유형이 오답 선지로 등장.
