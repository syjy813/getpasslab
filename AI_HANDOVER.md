...
>
> **최초 작성일**: 2026-07-14 / **최종 갱신일**: 2026-09-24 / **작성자**: Claude·ChatGPT·Codex / **버전**: v1.29

---


## 최신 보조 기출·화면 수정 — 2026-09-24

- 기준선: origin/main `f76a5e2883b361f3413256d99b8db338a35faa57`. 최초 8개 챕터 공개는 PR #81로 배포 완료. 아래 최초 공개 절의 배포 대기는 당시 기록
- 이번 작업: 컴활 챕터의 반복 공개 안내 배너 제거 및 기초 3개 챕터의 보조 기출 연결
- ADD-01 → 20200704_006, ADD-44 → 20180303_019, ADD-36 → 20150307_037. 기존 주 기출 배정 유지
- 기초 챕터만으로 완결되는 직접 기출이 아님. 적용 범위와 필요한 공개 설명 링크를 함께 표시. 주 기출 14개·공개 챕터 8개 유지, 보조 연결 3개는 빈도 집계에서 제외
- 관계 정본: 공개 챕터 frontmatter의 `supportingQuestions`. 원본 문제·정답·그림 변경 없음
- Test: 컴활 공개 범위·SEO·공개 콘텐츠·기존 지연 기출 검사 통과. 보조 3문항 열기·정답·닫기 확인
- Build: 268페이지·HTML 270개·sitemap 269 URL 성공
- 현재 단계: 구현 완료 → Test 완료 → Build 완료. 이번 변경의 Push·Deploy·Production은 이 스냅샷 작성 시 대기
- 상세 근거: `docs/audits/2026-09-24-foundation-question-links.md`
- 원본 저장소의 대규모 미커밋 작업 보존. 공개용 worktree에서 구현하며 원본 main에 무조건 pull/reset 금지

## 최초 공개 작업 상태 — 2026-09-24 (당시 기록)

- Owner가 승인한 컴활 2급 8개 챕터의 정식 공개 전환 구현 완료
- 공개 정본: `src/content/chapters/computer-literacy/written/`. 이전 로컬 초안은 원본 저장소에 보존하며 이후 공개 본문 수정은 공개 정본에서 진행
- 공개 경로: `/computer-literacy/written/`, 2개 과목, 8개 챕터. 홈·메뉴 연결 및 일부 공개 안내 포함
- 연결 기출 14문항, 검수 이미지 6개, 학습 SVG 4개. 미배정 기초 3개는 기출 미배정으로 표시. 부분 표본에서 빈출 배지 계산 제외
- 공개 작업은 `codex/publish-computer-literacy-8` 별도 worktree에서 수행. 원본 main의 대규모 미커밋 데이터·검수 도구·초안은 보존. 원본에서 무조건 pull/reset 금지
- Test: SEO·공개 콘텐츠·기존 deferred delivery·컴활 공개 범위 검사 통과. 390px 대표 기출 2개 이미지·정답·닫기·챕터 이동 확인
- Build: 2026-09-22 성공, 268페이지·HTML 270개·sitemap 269 URL. 2026-09-24 동일 산출물 검사 재통과
- 현재 단계: 구현 완료 → Test 완료 → Build 완료 → Push/Deploy/Production 대기. 완료 여부는 실제 GitHub 배포 및 실사이트 확인으로 갱신
- 과거의 비공개 준비 상태는 아래 이력에 보존. 현재 공개 전환 범위와 검증은 이 절 및 `docs/audits/2026-09-24-computer-literacy-publication.md` 우선

## ⚡ 30초 요약 (먼저 이것만)

**GetPassLab** = 자격증별 기출 역설계·출제기준 기반 핵심 요약 정적 웹사이트.

- **스택·공개 범위:** Astro 7 + Markdown + GitHub Pages. 기존 공개 서비스는 산업안전기사·에너지관리기능사이며 컴활 2급은 비공개 준비 단계다.
- **실제 개발 저장소:** `C:/Users/Guns/Documents/GitHub/getpasslab`. 웹 GetPassLab 프로젝트의 로컬 미러는 참조 컨텍스트이며 실제 저장소와 구분한다.
- **현재 Git 기준선:** `main`, `c7bfe0bd96f574c5540e110b7ec7fe709d78385a`. 2026-09-16 원격 조회에서 origin/main과 일치했다. 원격 브랜치 85개도 로컬 추적 정보와 일치했다. 이후 자동 commit·push·merge·배포 없음.
- **보존할 로컬 작업:** 복구 시 수정 5개·untracked 157개. 컴활 560문항·이미지 등록 139개·회차별 검수 화면·목차/감사 자료·pilot-01 초안 5개가 포함됐다. 이번 pilot-02 초안 3개·보고서 1개와 이 문서 갱신을 더했다. 상세 상태는 문서 말미 2026-09-16 절을 우선한다.
- **최신 로컬 UI (2026-09-16):** 컴활 초안 8개를 기존 ChapterLayout·BaseLayout·QuestionHistory에 연결했다. 로컬 홈의 컴활 카드 → 2과목 목차 → 챕터·기출 팝업으로 접근한다. 시작 URL `http://127.0.0.1:4321/admin/computer-literacy-learning/`. DEV 전용이며 공개 Build에서는 페이지·진입 링크 모두 제외된다. 콘텐츠의 검수 상태를 완료로 올리지 않았다.
- **최신 콘텐츠 수정 (2026-09-19):** 초안 8개를 최종 요청인 ‘명사형·~함·~임, 문장 끝 마침표 없음’으로 통일하고 첫 문단을 화면 요약으로 사용해 중복 서론을 제거했다. 목표값 찾기는 ‘문제 단서 → 셀·목표값 대응 → 화면 의미 해석 → 오답 제거’로 재구성했다. 주 5문항의 적용 기준을 대조했으며 실제 독자 정답률을 측정한 것은 아니다. 새 집필 기준은 아래 최신 절을 따른다.
- **최신 시각 설명 (2026-09-19):** 목표값 찾기의 셀 찾기·설정 해석 표 2개를 워크시트·설정창 SVG로 교체했다. ① 평균 E3, ② 문제의 목표 70, ③ 컴퓨터 판매량 B3을 연결한다. 학습용 재구성으로 명시했으며 실제 Excel 실행 캡처가 아니다. 390px·1280px UI 확인 및 11:49 KST Build 통과. 아래 시각 설명 절 참조.
- **최신 집필 기준·축약 (2026-09-19):** 전체 챕터에 `CHAPTER_WRITING_GUIDE.md` 적용. 최우선 원칙은 **기출을 푸는 데 굳이 필요 없는 내용은 넣지 않는다**. 목표값 찾기는 반복 표 3개와 마지막 조건 절을 제거하고 수동 소제목 5개를 2개로 축소했다. 주 5문항의 판단 근거와 그림 2장은 보존했다. 나머지 7개도 같은 기준으로 편집했으며 2026-09-22 최종 UI·Build 검증을 완료했다. 범위는 현재 작성된 컴활 초안 8개이며 설계 277개 신규 집필 완료가 아니다.
- **현재 콘텐츠 진행:** 통합 집필 목차 277개는 설계다. 비공개 본문 초안은 pilot-01 5개 + pilot-02 3개 = 8개. 새 공개 ID·slug·관계는 생성하지 않았다. 이름 범위의 버전·입력 조건과 실제 Excel 실행은 검증이 남는다.
- **최근 검증:** 2026-09-22 22:15 KST Build 256페이지·HTML 258개, SEO 257 URL 경고/오류 0, 공개 콘텐츠·deferred 검사 통과. 컴활 초안 8개 390px 화면·기출 14개 연결 확인. 기존 500kB 청크 경고와 Excel 실제 실행 미검증 상태 유지.
- **원격 배포 근거:** HEAD의 GitHub build·deploy는 2026-09-02 성공. 이번 복구에서 최신 Pages 배포도 같은 SHA임을 확인했으나 현재 브라우저 Production QA는 하지 않았다. 미커밋 컴활 작업은 Push·Deploy·Production 완료가 아니다.
- **별도 보존:** feature/chapter-search에는 현재 원격 브랜치에 없는 e07c99f·385a384 두 커밋이 있다. 현재 main보다 2개 앞서고 39개 뒤처진 별도 작업이다. docs/update-handover-production-qa worktree도 남아 있다.
- **외부 상태:** Search Console·AdSense 상태는 이번에 재확인하지 않았다. 아래 8월 22일 기록을 현재 상태로 단정하지 않는다.

상태는 **계획 → 작업 중 → 구현 완료 → Test → Build → Push → Deploy → Production**으로 구분한다. 새 작업 전 branch·HEAD·git status·이 문서·목적을 확인한다. 아래 과거 절은 당시 기록이며 현재 스냅샷과 다르면 실제 파일·Git 상태를 우선한다.
...
페이지의 기본 구성은 다음과 같다. 수동 본문은 `CHAPTER_WRITING_GUIDE.md`에 따라 기출 풀이에 필요한 부분만 작성하며, 아래 형식을 채우려고 설명이나 소제목을 추가하지 않는다.
...
- 대상 공사 착공 전날까지 제출.
- 공사 중 6개월 이내마다 확인.
- 현행 시행규칙 별표 10의 공통 첨부서류 6종, 모든 대상 공사의 추가서류 2종과 공사 유형별 세부 작업계획을 구분.
...
- 2026년 KEC 전문의 211.2.4에서 금속제 외함 저압 기계기구의 50V 초과 기준과 예외가 유지됨을 직접 확인했다. 실제 적용에는 전로·기기·장소 조건과 예외를 함께 확인한다.
...
## Gate B2 공식 원문 후속 보완 production 게이트 완료 (2026-08-03)

기존 Gate B2 병합 범위에서 현행 시행규칙 별표 10의 첨부서류 구분이 충분히 명시되지 않은 점을 후속 보완했다.

### 1. 구현·병합

- 콘텐츠 PR #36 `content: clarify construction plan attachments`.
- 현행 시행규칙 별표 10의 공통 첨부서류 6종, 모든 대상 공사의 추가서류 2종, 공사 유형별 세부 작업계획을 명시.
- PR HEAD: `75b8371e467c0224ca2364e927071a5803c46032`.
- ChatGPT 최종 기술 리뷰 PASS, review ID `4844512135`.
- SEO validation run `30818027694`, job `91700536765`: 성공.
- Squash merge commit: `cf2c88a134f89d44270765d05e98705182622a05`.

### 2. 공식 원문 확인

- 국가법령정보센터의 2026년 8월 1일 시행 「산업안전보건법 시행규칙」 별표 10 전문 직접 대조.
- 대한전기협회 공식 공고의 2026년 KEC 전문 HWP 직접 대조.
- KEC 211.2.4의 50V 초과 기준과 예외, KEC 234.5의 15mA·0.03초 또는 3kVA 이하 절연변압기 조건이 현행 전문에 유지됨을 확인.
- `20220424_065` 관계 유지 조건 충족.

### 3. 검증·production

- 전체 1,680문항, ID 중복 0건, 존재하지 않는 관계 ID 0건.
- Astro build 209페이지, SEO 경고·오류 0건, 공개 콘텐츠 오류 0건.
- GitHub Pages run #99 `30819046455`: `completed / success`.
- 건설공사와 누전차단기 대표 production 페이지 HTTP 200 및 법령 문구·관계 문항 확인.
- 복원된 미매핑 `20190804_119`가 건설공사 페이지에 노출되지 않음을 확인.
- 신규 `slug`, 동결 키, 문제 데이터, 관계, 스키마, 의존성, URL 규칙, production 설정 변경 없음.
- 상세 기록: `docs/audits/2026-08-03-gate-b2-official-source-followup-production-verification.md`.

Gate B2 최종 상태: **VERIFIED**. 완료 범위를 다시 수행하지 않는다.

### 4. 남은 위험과 다음 작업

- npm 취약점 5건과 도구·Actions 경고는 별도 의존성·CI T0 감사 대상으로 유지.
- AdSense 승인·실제 광고 송출, Search Console 색인·sitemap 처리, GA4 실시간 이벤트는 외부 계정 증거로 별도 확정.
...
## 에너지관리기능사 기반·핵심요약 production 게이트 완료 (2026-08-04)

다자격증 정적 콘텐츠 기반을 도입하고 에너지관리기능사 필기 핵심요약 12개를 production에 공개했다.

### 1. 구현·병합

- PR #49: 자격증·시험·과목 범위의 설정·라우팅·관계 조회 일반화. merge `24e485292b8be44f526699dfbf35ee4625f2526a`.
- PR #50: 에너지관리기능사 `energy-management`, 필기과목 `thermal-equipment`, 완료 챕터 12개 추가. merge `ebd35c37da892e676abd25f85beaecb0fd84f076`.
- Q-Net 2026~2028 출제기준의 필기 주요 항목 12개에 맞춰 보일러·연소·효율·제어·배관·운전·수질·안전·법규를 구성했다.
- 에너지관리기능사 기출 원본은 이번 범위에 없으므로 신규 기출 관계는 0건이다.
- 기존 산업안전기사 URL·동결 키·문제 데이터·챕터 원문, 의존성과 production 설정은 변경하지 않았다. 컬렉션 스키마에는 기존 데이터와 호환되는 기본값 포함 `cert_id`·`exam` 필드를 추가했다.

### 2. 검증·production

- 전체 1,680문항, ID 중복 0건.
- 전체 262챕터 중 완료 228개, 미시작 34개.
- Astro build 242페이지, SEO HTML 242개·sitemap URL 243개·경고 0개·오류 0개, 공개 콘텐츠 오류 0개.
- PR CI: #49 run `30909570504`, #50 run `30911082018` 모두 성공.
- GitHub Pages run `30911145877`: `completed / success`.
- 에너지관리기능사 허브·과목·대표 챕터, 기존 산업안전기사 대표 페이지와 sitemap HTTP 200·본문·canonical 확인.
- 상세 기록: `docs/audits/2026-08-04-energy-management-foundation-content-production-verification.md`.

최종 상태: 에너지관리기능사 기반·핵심요약 production 게이트 **VERIFIED**. 완료 범위를 다시 수행하지 않는다.

### 3. 남은 위험과 다음 작업

- 에너지관리기능사 기출·정답 원본은 아직 확보·검증되지 않았다. 원본 확보 뒤 데이터와 챕터 관계를 별도 게이트로 추가한다.
- 기존 산업안전기사 기출문제·이미지의 상용 이용 권리 위험은 해소되지 않았다.
- 법령·검사제도 콘텐츠는 2026-08-04 확인 기준이며 실제 적용 때 최신 원문·기기 조건·예외를 재확인한다.
- `esbuild` 설치 스크립트 allowlist 안내와 Actions 런타임 경고는 별도 의존성·CI 범위다.
...
## 산업안전기사 시각자료·컴활 2급 비공개 내재화 production 게이트 완료 (2026-08-09)

산업안전기사의 누락 시각자료를 원본 기준으로 보완하고, 컴퓨터활용능력 2급 1회차 데이터는 Git 정본에만 내재화한 채 production 공개를 차단했다.

### 1. 구현·병합

- PR #63: 산업안전기사 문항 이미지 75개와 자산 레지스트리, 기존 9개 챕터의 시각자료 10개, 컴활 2급 2020-07-04 회차 40문항과 이미지 16개, 개발 전용 검수 화면과 감사 자료를 추가했다.
- PR CI run `31306646526`, validate job `93227976108`: success.
- ChatGPT 최종 기술 리뷰 PASS 후 Squash merge했다.
- main 병합 커밋: `f200fe8a9ca865cc77473c5d4e40e4d66bd5ae96`.
- 컴활 2급 미리보기 `src/pages/admin/computer-literacy-preview/[...path].astro`는 개발 환경에서만 정적 경로를 생성한다.
- production 공개 검사에서 컴활 경로명, 자격증명, `20200704_###` 문항 ID를 전체 산출물 기준으로 차단한다.
- 신규 의존성·스키마·공개 URL 규칙과 기존 `slug`·`subject_id`·`question_id`는 변경하지 않았다.

### 2. 검증·production

- 산업안전기사 1,680문항, 중복 ID 0건, 변경 대상 76건, 동결 필드 위반 0건, 문항 이미지 레지스트리 75건의 자산 누락 0건.
- 컴활 2급 40문항, 중복 ID 0건, 번호 1~40, 이미지 레지스트리 16건의 자산 누락 0건.
- `npm ci`, Astro build, SEO 검사와 공개 콘텐츠 검사 모두 성공했다.
- Astro build 정적 HTML 255개, SEO sitemap URL 256개, 경고·오류 0건.
- production 산출물에서 `computer-literacy`, `컴퓨터활용능력`, `20200704_###` 표식 0건.
- GitHub Pages run `31306751013`: build job `93228236249`, deploy job `93228284554` 모두 success.
- production FTA·안전표지 페이지와 연결 이미지 자산, sitemap이 HTTP 200이며 새 시각자료가 본문에 포함됐다.
- production `/admin/computer-literacy-preview/`는 HTTP 404로 확인했다.
- 상세 기록: `docs/audits/2026-08-08-industrial-question-image-remediation.md`, `docs/audits/2026-08-08-computer-literacy-pilot.md` 및 동반 manifest.

최종 상태: 산업안전기사 시각자료 공개와 컴활 2급 비공개 내재화 게이트 **VERIFIED**. 컴활 2급은 별도 Owner 승인 전 production에 공개하지 않는다.

### 3. 남은 위험과 다음 작업

- 산업안전기사 이미지 후보 중 기존 챕터와 연결되지 않은 41건은 저장만 유지한다. 검증 없이 관계를 임의 추가하지 않는다.
- 기출 문제 텍스트와 이미지의 이용 권리 위험은 해소되지 않았으며 Owner 법무·사업 결정이 필요하다.
- 컴활 2급은 1회차만 내재화했다. 추가 회차는 원본 PDF 단위로 별도 검수·승인한다.
- AdSense 승인·실제 광고 송출, Search Console 처리와 GA4 실시간 이벤트는 외부 계정 증거로 별도 확정한다.

다음 작업은 산업안전기사 미연결 726문항을 위험 기반 배치로 재검토한다. 먼저 기존 챕터에 거의 일치하지만 본문 범위·검사기 문제로 보류한 20문항을 정밀 검토하고, 이후 REVIEW 후보를 과목별로 진행한다. 신규 공개 slug는 Owner 승인 없이 생성하지 않는다.

## 전 공개 챕터 시험 포인트 전수 개선 production 게이트 완료 (2026-08-13)

퇴근 후 짧은 시간에 공부하는 40대 직장인 수험자 관점에서 공개 챕터의 학습 내용을 재검토하고, 문제에서 확인해야 할 표현과 정답 키워드를 빠르게 연결할 수 있도록 전수 개선했다. 개념과 판단 이유는 유지했으며 기출에 나온 정보만을 삭제 기준으로 삼지 않았다.

### 1. 구현·배포

- 산업안전기사 218개와 에너지관리기능사 21개를 합친 공개 챕터 239개에 `시험 포인트` 표를 적용했다.
- 시험 포인트는 회차·정답 번호 설명보다 `문제에서 보이는 표현 → 정답으로 연결` 구조를 우선했다.
- 정답이 되는 용어, 숫자·단위, 방향, 조건·예외를 선별해 볼드 처리했다.
- 설명 문단과 표·목록은 의미를 유지하면서 짧은 명사형 표현으로 정리했고 불필요한 종결 마침표를 제거했다.
- 시험 포인트가 12행 이상인 장문 챕터 13개는 정보 삭제 없이 `우선 암기 / 추가 확인`으로 분리했다.
- 전체 감사 스크립트와 결과를 `scripts/audit-exam-point-coverage.mjs`, `scripts/normalize-chapter-style.mjs`, `docs/audits/2026-08-13-exam-point-all-chapters-audit.{csv,md}`, `docs/audits/2026-08-13-exam-point-qa-pilot.md`에 기록했다.
- main 배포 커밋: `54132c4efce98624f65869c128cba35ae64844e8` (`feat: sharpen exam-focused chapter guidance`).
- GitHub Pages run `31703403434`: build job `94457826393`, deploy job `94458009815` 모두 success.

### 2. 검증된 현재 상태

- 공개 챕터 239개, 시험 포인트 섹션 239개, 시험 포인트 1,647행.
- 연결 기출 참조 2,507개, 표·강조 누락 0개, 누락 기출 ID 0개 챕터.
- 잘못된 시험 포인트 표 행 0개, 정답 칸 볼드 누락 0개.
- 기존 `slug`, `subject_id`, `questions` 관계와 기출 데이터 변경 0건.
- Astro build 253페이지 성공.
- 공개 콘텐츠 검사: HTML 255개, 오류 0개.
- SEO 검사: HTML 255개, sitemap URL 254개, 경고 0개, 오류 0개.
- production `tunnel-blasting`, `protective-equipment-types`, `boiler-protection-devices` 대표 3페이지가 HTTP 200이며 `우선 암기 / 추가 확인` 신규 구조 노출을 확인했다.
- 배포 직후 로컬 `main`과 `origin/main`은 위 커밋으로 일치했고 working tree는 깨끗했다.

최종 상태: 전 공개 챕터 시험 포인트 전수 개선 게이트 **VERIFIED**. 같은 기계적 전수 변환을 다시 수행하지 않는다.

### 3. 남은 위험

- 감사 휴리스틱상 `boiler-water-treatment` 1개 챕터가 시험 포인트 키워드 대조율 25% 미만이며, 12개 챕터가 본문 정답 근거 대조율 50% 미만이다. 이는 자동 문자열 대조 후보이며 콘텐츠 오류로 확정된 상태가 아니다. 상세 후보는 `docs/audits/2026-08-13-exam-point-all-chapters-audit.{csv,md}`를 정본으로 사용한다.
- 전체 239개 production 페이지의 육안 QA는 수행하지 않았다. 자동 산출물 전수 검사와 대표 3페이지 runtime 검증만 완료했다.
- 법령·수치·수식·부정문·조건·예외는 자동 문체 변환 결과만 신뢰하지 않고 후속 의미 QA에서 직접 확인한다.
- 기출 문제 텍스트와 이미지의 이용 권리 위험은 해소되지 않았으며 Owner 법무·사업 결정이 필요하다.
- Owner가 AdSense 재검토를 요청했다고 보고했으나 승인·광고 송출 여부와 Search Console·GA4 상태는 외부 계정 증거로 다시 확인해야 한다.

### 4. 다음 작업

1. 감사 후보 13개는 전수 재작성하지 말고 실제 의미 누락이 있는 항목만 수동 QA한다.
2. 법령·수치·수식·부정문·조건·예외 챕터는 위험 기반 배치로 직접 검토한다.
3. AdSense 심사 결과가 나오기 전에는 대규모 IA·검색·광고 구조 변경보다 현재 production 안정성과 콘텐츠 품질을 유지한다.
4. 외부 우선순위는 AdSense 심사 결과, Search Console 색인·sitemap 처리, GA4 실시간 이벤트 확인이다.

---

## 산업안전기사 기출 관계 정밀 보강 게이트 완료 (2026-08-16)

기존 산업안전기사 1,680문항을 새로 수집하지 않고, 보유 기출의 활용도를 높이기 위해 미연결 문항과 기존 공개 챕터의 관계를 재검토했다. 자동 후보를 그대로 반영하지 않고 문항 의미·챕터 범위·법령·이미지·공개 콘텐츠 검사를 다시 확인해 검증된 관계만 반영했다.

### 1. 구현 결과

- PR: `#66 fix: add verified industrial safety question relations`
- merge commit: `0998dc439f8abfc1a20010f731092a1cf8307cea`
- 기존 공개 챕터 18개에 기출 관계 **23건** 추가
- 신규 `slug`, `subject_id`, `question_id` 변경 없음
- 신규 의존성·스키마·UX 변경 없음
- `anthropometry-design`, `severity-rate`의 관계 수에 맞춰 `examComment` 수치 보정
- 기존 `total-noise-dose`의 공개 HTML에 Markdown 강조 마커가 남던 렌더링 오류 1건을 의미·수치 변경 없이 수정
...
- 미연결 문항: **726**
- 다중 연결 문항: **10**
- 최초 ADD 후보 43건 중 19건은 챕터 범위 불일치·본문 미수록으로 제외
- `20190303_094 -> spontaneous-combustion` 1건은 의미상 관계가 맞지만 `check-public-content.mjs`가 정상 시험 용어 `저장소`를 금지어로 오탐하여 관계 반영을 보류

### 3. 검증·배포 증거

- 관계 dry-run 및 question_id 무결성: PASS
- `npm run build`: PASS — **255 pages**
- `npm run check:seo`: PASS — HTML **257**, sitemap URL **256**, 경고 **0**, 오류 **0**
- `npm run check:public-content`: PASS — HTML **257**, 오류 **0**
- merge 후 GitHub Pages run `31892749968`: build **success**, deploy **success**
- 실제 라이브 페이지의 별도 브라우저 runtime QA는 이번 게이트에서 수행하지 않음

### 4. 남은 위험·다음 작업

- 미연결 **726문항**은 추가 검토 대상이며, 관계 수를 늘리기 위해 억지 매핑하지 않는다.
- 우선 검토 대상은 기존 ADD에서 보류된 **20문항**이다. 19건은 기존 챕터 범위·본문 보강 가능성을 다시 판단하고, `20190303_094`는 공개 콘텐츠 검사기의 false positive를 별도 범위에서 다룬다.
- 신규 챕터·신규 공개 slug가 필요하면 별도 Owner 결정으로 분리한다.
- `npm ci`에서 기존 의존성의 **2 high severity vulnerabilities** 경고가 확인됐으나 이번 작업에서 의존성을 변경하지 않았다.
...
## 2026-09-16 로컬 상태 복구·컴활 pilot-02 시범 집필

### 기준선과 기존 작업 보호

- 실제 저장소 `C:/Users/Guns/Documents/GitHub/getpasslab`, branch `main`, HEAD `c7bfe0bd96f574c5540e110b7ec7fe709d78385a`.
- 변경 전 기존 파일 683개를 해시로 기록했다. AI_HANDOVER 갱신 외 기존 682개는 유지한다. 기존 수정 5개·미추적 157개를 폐기·덮어쓰기하지 않았다.
- 이번 신규 추적 대상: `docs/drafts/computer-literacy/pilot-02/`의 P1-39·P2-44-03·P2-20과 `docs/audits/2026-09-16-computer-literacy-pilot-02-review.md`. AI_HANDOVER를 포함해 이번 변경 5개 파일.
- 완료 후 예상 상태: 수정 6개·untracked 161개·staged 0개. 실제 값은 매 작업 시작 시 다시 확인한다.
- raw_pdfs·tmp는 Git 무시 대상이다. 작업 자료가 있으므로 삭제·자동 정리하지 않는다. 별도 로컬 검색 브랜치·문서 worktree도 보존한다.

### 복구된 최근 작업

- 마지막 커밋은 둥근톱 용어 정리. 그 앞에 둥근톱 안전 설명, Kakao AdFit 반응형 광고, 홈페이지 섹션 재배치 커밋이 있다.
- 컴활 DB는 HEAD 40문항에서 로컬 560문항으로 확장됐다. 과목별 280문항, 14회차 각각 40문항. 이미지 등록은 16→139개. 기존 40문항·이미지 등록 16개 내용은 동일하다.
- PDF 수집 도구와 DEV 전용 회차별 검수 화면에 미커밋 변경이 있다. 공식 출제기준 대응·사실 검토·277개 통합 집필 목차는 docs에 보존돼 있다.
- pilot-01은 비공개 초안 5개와 연결 7문항 검토까지 진행됐다. pilot-02는 9월 15일 선정 자료만 있던 상태에서 이번에 본문 3개와 보고서를 추가했다.

### 이번 결과와 한계

- P1-39: 선택 항목·활성 창·작업 표시줄의 단축키 차이. 과거 버전 설명을 현행 UI와 구분한다.
- P2-44-03: 이름을 주소로 치환하고 COUNT·AVERAGE·SUMPRODUCT·INDEX를 읽는 흐름. 범위 덧셈은 암시적 교차·동적 배열 조건을 명시하며 실제 실행 대기 상태를 유지한다.
- P2-20: 수식 셀·목표 숫자·변경 입력 셀, 입력→결과 비교와 결과→입력 역산의 구분.
- 연결 문항 주 10·보조 5, 총 15개. 제공 교사용 PDF 11개·14쪽의 대상 문항과 등록 이미지 9개 대조. 원본의 채점 표기와 정답표·DB 일치. 원출제기관의 정정 확인과는 별개다.
- 기존 이름 범위 문항 20150307_037의 조건부 확인은 유지한다. 기존 미해결 20200229_040·20160305_039 이미지와 20200704_024 입력 재현은 수정하지 않았다.
- Excel COM 자동화가 RPC_E_CALL_REJECTED로 완료되지 않아 성공한 Excel 실행 검증은 0건이다. 독립 산술을 실제 제품 실행으로 기록하지 않았다. 검증용 임시 Excel 프로세스만 생성 시각·명령줄 확인 후 종료했다.

### 단계별 상태

| 단계 | 2026-09-16 확인 |
| --- | --- |
| 계획 | 기존 선정 자료의 pilot-02 범위 3개 확인 |
| 작업 중 | 컴활 전체 집필·실행 검증·공개 준비는 계속 진행 중 |
| 구현 완료 | 비공개 pilot-02 초안 3개·검수 보고서 작성. 전체 277개 완료 아님 |
| Test | 독립 산술 17건·문서·원본 대조·파일 보존 검사 통과. Excel/Windows 실기 성공 없음 |
| Build | 로컬 Astro 256페이지, HTML 258개. SEO·공개 콘텐츠·deferred 검사 통과. 청크 경고 유지 |
| Push | 이번 작업 미실행. 기존 HEAD만 원격 반영 확인 |
| Deploy | 이번 작업 미실행. 기존 HEAD의 9월 2일 Pages 성공 기록만 확인 |
| Production | 컴활 신규 작업 미반영. 현재 운영 브라우저 QA 미실시 |

### 다음 작업

1. 대상 Excel에서 버전·지역 설정·입력 위치·일반/배열 입력 방식과 실제 결과를 기록해 이름 범위·목표값 찾기를 검증한다. 실패하면 조건부 상태를 유지한다.
2. 누적 비공개 시범 초안 8개의 설명 흐름·중복을 검토한다. 공개 페이지 수·ID·slug·관계는 별도 범위 확정 후 진행한다.
3. 기존 이미지 2건·입력 재현 1건을 별도 작업으로 해결한다. 로컬 전용 검색 커밋 2개도 새 작업과 섞지 않는다.

상세: [pilot-02 검수 보고서](docs/audits/2026-09-16-computer-literacy-pilot-02-review.md). 재현 자료: `tmp/pilot02-computer-literacy/2026-09-16/`. 자동 commit·push·merge·배포는 하지 않았다.

---

## 2026-09-16 컴활 초안의 기존 UX/UI 로컬 적용

### 목적과 구현

- Owner 요청: 별도 임시 Markdown 검토 화면으로는 콘텐츠 판단이 어려우므로 실제 GetPassLab UX/UI에 적용.
- main / c7bfe0bd96f574c5540e110b7ec7fe709d78385a에서 기존 미커밋 작업을 보존하며 구현했다.
- 기존 BaseLayout·ChapterLayout·GNB·QuestionHistory·디자인 토큰을 재사용했다. 별도 프레임워크·패키지 추가 없음.
- 로컬 홈과 상단 자격증 메뉴에서 컴활로 진입한다. 필기 허브 1개·과목 목차 2개·본문 8개, 합계 11개 DEV 경로.
- 주소는 `/admin/computer-literacy-learning/{과목번호}/{기존 초안 ID}/`. 공개 slug를 확정하거나 CERTIFICATIONS 공개 등록을 변경하지 않았다.
- 본문·제목은 docs/drafts의 기존 원본을 읽는다. 요약·주 문항 ID는 기존 통합 집필 목차를 읽는다. 편집 메모는 학습 화면에서 제외했다. 원본 초안·문항·배정·이미지 파일 변경 없음.
- 주 문항 14개, 이미지 6개를 기존 기출 팝업으로 표시한다. 보조 문항을 직접 출제 집계에 더하지 않았다. 미배정 ADD 초안 3개는 미배정이라고 표시한다.
- 과목 전환·현재 챕터 강조·모바일 챕터 목록·이전/다음 이동·같은 과목의 다른 챕터 링크를 제공한다. 넓은 표만 내부 가로 스크롤을 사용한다.
- 로컬 이미지의 data URL을 QuestionHistory에서 원본 크기 정보와 함께 표시하도록 확장했다. 기존 공개 이미지·deferred 경로는 유지했다.

### 확인 결과

- Test: 11개 경로 응답·제목 1개·noindex·편집 메모 제외·목차 링크 정합성·14개 팝업·이미지 6개 검사 통과. 미등록 초안 URL은 404.
- 브라우저: 이름 범위 본문, 기출 이미지 로딩, 정답 공개·닫기, 모바일 목록, 다음 챕터, 과목 전환, 로컬 홈 동선 확인.
- 반응형: 390px에서 페이지 가로 넘침 없음(표는 내부 스크롤). 1280px에서 좌측 목차와 현재 챕터 강조 확인. 임시 화면 크기 설정은 검사 후 해제.
- Build: 2026-09-16 22:59 KST 성공, Astro 256페이지·HTML 258개. 기존 500kB 청크 경고 유지.
- SEO: HTML 258개·sitemap URL 257개, 경고/오류 0. 공개 콘텐츠 검사와 deferred 기출 검사 통과.
- 공개 Build 결과에서 컴활 미리보기 경로·진입 링크·로컬 안내가 없음. 기존 공개 콘텐츠 페이지 수 유지.
- 변경 전 687개 파일 해시 기록. 이번 의도된 기존 파일 5개(레이아웃·GNB·QuestionHistory·홈·인계 문서) 외 682개 보존. 신규 구현 파일 3개.

### 단계와 다음 작업

계획·로컬 구현·UI Test·Build 완료. 자동 commit·Push·Deploy 없음. Production에 컴활은 반영되지 않았다.
콘텐츠 자체는 검토 중인 8개 초안이다. 실제 Excel 재현과 과거 Windows 조건 검증의 기존 미완료 상태를 유지한다.
Owner는 기존 사이트 화면에서 설명 흐름·분량·표·기출 연결을 검토할 수 있다. 이 UI 적용을 콘텐츠 최종 승인이나 전체 277개 집필 완료로 해석하지 않는다.

현재 로컬 실행: `npm run dev -- --host 127.0.0.1 --port 4321`.
검토 시작: `http://127.0.0.1:4321/admin/computer-literacy-learning/`.
이름 범위 챕터: `http://127.0.0.1:4321/admin/computer-literacy-learning/2/P2-44-03/`.
검증 자료: `tmp/computer-literacy-ui/2026-09-16/` (Git 무시 대상).

---

## 2026-09-19 문체 통일·목표값 찾기 기출 적용 개선

### Owner의 콘텐츠 기준

전체 챕터의 최신 포함·삭제 기준은 [CHAPTER_WRITING_GUIDE.md](CHAPTER_WRITING_GUIDE.md)를 따른다. **기출을 푸는 데 굳이 필요 없는 내용은 넣지 않는다.**

1. 학습 본문은 최종 Owner 요청(2026-09-19)에 따라 **명사형·~함·~임, 문장 끝 마침표 없음**으로 통일한다. 이전 ‘~한다/이다’체 요청을 대체한다. 소수점·파일 확장자·수식과 원본 기출은 보존한다.
2. 반복과 불필요한 부연을 줄인다. 정확성에 필요한 조건·버전·판단 과정은 남긴다.
3. 최우선 목표는 독자가 이론을 보고 바로 기출에 적용하는 것이다. 용어 정의만으로 집필 완료를 판단하지 않는다.
4. 연결 기출의 요구를 먼저 분류하고, 본문에서 **찾을 단서 → 적용 순서 → 정답 선택·오답 배제 근거**를 확인한다.
5. 문제가 설정이나 의미를 물으면 그 판단에 집중한다. 직접 계산을 묻지 않는데 역산값 계산을 주요 과제로 내세우지 않는다.

### 구현 범위

- pilot-01 5개·pilot-02 3개, 총 8개 학습 본문 편집. 제목·기존 출처와 검수 이력을 보존했다.
- 첫 문단을 미리보기의 요약으로 사용하고 본문에 중복 출력하지 않는다. 기존 기출 배정은 통합 집필 목차에서 계속 읽는다.
- 컴활 기출 안내는 간결한 서술형으로 표시한다. QuestionHistory에 선택적 introText를 추가하고 다른 공개 페이지는 기본 문구를 유지했다.
- Markdown 강조가 문자로 노출되던 4곳을 수정했다. 본문 기준 문자 수는 15804→9795(38.0% 감소, Markdown 표기 포함·편집 메모 제외). 축약률은 교육 효과의 증거가 아니다.
- 목표값 찾기 주 5문항: 도구 선택 1·설정값 선택 2·화면 의미 해석 2. 평균 예시에 행 번호·열 문자·항목명을 넣고, 조건문에서 E3·70·B3를 찾는 과정을 설명했다.
- 목표 숫자와 현재 값 혼동, 결과와 입력 반전, 다른 품목 선택, 여러 입력 변경의 오답 기준을 설명했다. 원본 기출의 숨은 수식은 추정하지 않았다.
- 검토 보고서: `docs/audits/2026-09-19-computer-literacy-goal-seek-learning-review.md`.
- 기출 DB·정답·이미지·관계 목차·공개 콘텐츠 원본 변경 없음. 보조 문항의 주 문항 편입이나 미배정 초안의 신규 배정 없음.

### 검증 상태

- Test: 초안 8개 문체·제목·이전 편집 메모 보존·중복 서론·강조 렌더링 확인. 11개 로컬 경로·14개 기존 기출 팝업 연결 확인.
- 브라우저: 목표값 찾기 본문과 2020년 7월 23번의 기출 팝업·정답 공개·닫기 확인.
- Build: 2026-09-19 11:21 KST 성공, 256페이지·HTML 258개. 기존 청크 크기 경고 유지.
- SEO: HTML 258개·sitemap URL 257개, 경고/오류 0. 공개 콘텐츠·deferred 검사 오류 0. 컴활 미리보기는 운영 산출물에서 제외됨.
- 문체 작업 시작 시 기록한 기존 파일 690개 중 의도된 11개(초안 8·페이지·QuestionHistory·인계 문서) 외 679개 보존. 감사 문서 1개 신규.
- main / c7bfe0bd96f574c5540e110b7ec7fe709d78385a 유지. Commit·Push·Deploy 미실행, Production 반영 없음.
- 기존 Excel 실제 실행 미검증·과거 Windows UI 조건부 상태 유지. 이번 검토는 본문-문항 대응이며 실제 학습자 정답률 검증은 아니다.

로컬 검토: `http://127.0.0.1:4321/admin/computer-literacy-learning/2/P2-20/`.
검증 자료·편집 전 본문 백업: `tmp/content-tone/2026-09-19/`.
다른 7개 초안은 이번에 문체·분량만 정리했다. 목표값 찾기에 적용한 기출별 풀이 대응 검토가 모두 끝난 것으로 해석하지 않는다.


---

## 2026-09-19 목표값 찾기 시각 설명 전환

- Owner 요청: 표보다 화면 이미지가 적합한 항목을 검토한 뒤 우선 목표값 찾기에 적용한다.
- `P2-20.md`의 셀 위치·설정 해석 표 2개와 중복 설명을 번호로 연결한 그림 2장으로 교체했다. 문제 조건 → 워크시트 → 설정창 → 의미 해석 순서다. 오답 비교·도구 비교 표와 주 기출 5문항 연결은 유지한다.
- ① E3의 현재 평균 60은 수식 결과, ② 70은 문제에서 제시한 목표, ③ B3의 50은 변경할 입력이다. 목표 숫자가 같은 C3의 프린터 판매량과 혼동하지 않도록 본문에 구분을 남겼다.
- 신규 원본 SVG 2개: `docs/drafts/computer-literacy/pilot-02/illustrations/goal-seek-sheet.svg`, `goal-seek-settings.svg`. 수식·좌표가 정확한 학습용 재구성이다. 실제 Excel 캡처·실행 결과로 표시하지 않는다.
- DEV 미리보기 페이지가 초안에 참조된 같은 pilot의 그림만 data URL로 삽입한다. 파일명은 영문 소문자·숫자·하이픈의 SVG로 제한한다. public 디렉터리나 공개 콘텐츠에 복제하지 않는다.
- 반응형은 기존 디자인 토큰을 사용한다. 데스크톱은 두 그림을 나란히, 모바일은 위아래로 배치한다. 번호·텍스트·대체 텍스트를 제공해 색만으로 의미를 구별하지 않는다.
- Test: SVG XML 파싱, 예시 평균 산술 확인. 390px/1280px 이미지 로드·배치·페이지 가로 넘침 없음 확인. 주 기출 버튼 5개, 2020년 7월 23번 팝업 이미지·정답 공개·닫기 확인. 임시 viewport 설정 해제.
- Build: 2026-09-19 11:49 KST 성공, 256페이지·HTML 258개. 기존 500kB 청크 경고 유지. SEO 257 URL 경고/오류 0, 공개 콘텐츠·deferred 검사 오류 0. 운영 HTML/SVG에서 로컬 경로·그림 참조·시각 설명 본문이 제외됨을 확인했다.
- 단계: 로컬 구현·UI Test·Build 완료. Commit·Push·Deploy 미실행, Production 반영 없음. 실제 Excel 실행·학습 효과 검증은 이번 작업의 결과가 아니다.
- 변경 전 파일 백업·적용 스크립트·빌드 로그: 작업 공간 `work/goal-seek-visual/`. 다음 시각화 후보는 INDEX 상대 위치와 SUMPRODUCT의 대응 셀이다. 아직 적용하지 않았다.


---

## 2026-09-19 명사형 문체·모바일 페르소나 검토

- Owner의 최신 문체: ‘대응시킨다.’ → ‘대응시킴’, ‘찾는 기능이다.’ → ‘찾는 기능’처럼 간결한 명사형과 문장 끝 마침표 제거
- 초안 8개 본문·요약·표·제목, 목표값 찾기 SVG 2개의 문구, 로컬 미리보기 안내에 적용
- 문장 간 줄바꿈을 유지해 마침표 없는 문구가 이어 붙지 않도록 처리
- 목표값 찾기의 바뀐 소제목 4개는 기존 앵커 ID를 유지해 이전 링크 보존
- 원본 기출 문장·정답·배정, 수식·소수점·파일 확장자, 출처·편집 메모 보존
- 모바일 사용성: 390×844에서 연결 기출 5개 풀이 근거 확인 후 정답 대조 일치. 이미 정답을 아는 AI의 점검이며 학습자 정답률 실험이 아님
- 개선 후보: 문제 이미지 확대·세 항목 대응 해설, 그림 직후 대표 문제, 다음 문제 이동·진행 표시, 모바일 표의 세로 묶음. 아직 구현하지 않음
- 상세 보고서: `docs/audits/2026-09-19-computer-literacy-mobile-persona-review.md`
- Test: 초안 8개 실제 화면에서 명사형·문장 끝 마침표 제거·강조 정상·h1 1개·noindex·페이지 가로 넘침 없음·기존 기출 14개 연결 확인. SVG 파싱, 출처 메모 보존, 소수점·확장자 보존 검사 통과. 임시 모바일 viewport 해제
- Build: 2026-09-19 12:05 KST 성공, 256페이지·HTML 258개. 기존 500kB 청크 경고 유지
- SEO 257 URL 경고/오류 0, 공개 콘텐츠·deferred 검사 오류 0. 공개 HTML에 컴활 미리보기·학습 그림 참조·본문 미포함
- 단계: 로컬 구현·Test·Build 완료. Commit·Push·Deploy 미실행, Production 미반영
- 변경 전 파일 백업·적용 스크립트·Build 로그: 현재 작업 공간 `work/nominal-tone/`


---

## 2026-09-19 기출 풀이 중심 축약·전체 챕터 작성 가이드

- Owner 요청: 불필요한 설명은 판단해 줄이고 ‘기출을 푸는 데 굳이 필요 없는 내용은 넣지 않는다’를 전체 챕터 집필 기준에 포함
- 신규 `CHAPTER_WRITING_GUIDE.md`: 모든 자격증의 신규 집필·기존 개정에 적용. 각 설명이 문제 이해·적용·정답 판단·오답 제거에 쓰이는지 확인하고 중복과 무관한 부연 제외. 정답을 바꾸는 조건과 필요한 기초 개념은 보존
- `CODING_RULES.md`의 콘텐츠 규칙에서 가이드 연결. 고정 글자 수를 맞추는 규칙을 풀이에 필요한 분량 기준으로 교체
- 본문 수정은 `docs/drafts/computer-literacy/pilot-02/P2-20.md` 한 챕터. 반복 셀 대응표·오답 표·도구 비교표와 마지막 조건 절 제거. 수동 소제목 5→2, 표 3→0
- 수식 결과·참조 입력 조건은 그림 설명에 통합. 오답 판단 2줄, 도구 비교 3줄 유지. 도구 선택은 실제 주 기출의 판단에 필요하므로 삭제하지 않음
- 그림 2장·기존 앵커 4개·주 기출 5개 연결 유지. 원본 기출·정답·배정·이미지·공개 콘텐츠 변경 없음. 이전 출처·편집 이력 보존
- Test: 390px에서 그림 2장 로딩·표 없음·가로 넘침 없음·앵커 4개·주 기출 버튼 5개 확인. 워크시트·설정창과 축약 캡션을 모바일에서 시각 확인. 임시 화면 크기 해제
- Build: 첫 시도는 실행 중인 미리보기 서버와 `.astro` 임시 파일 rename 충돌(EPERM)로 실패. 서버를 정지한 후 2026-09-19 22:08 KST 재실행 성공, 256페이지·HTML 258개. 기존 500kB 청크 경고 유지
- SEO: HTML 258개·sitemap URL 257개 경고/오류 0. 공개 콘텐츠·deferred 검사 오류 0
- 미리보기 서버를 빌드 후 127.0.0.1:4321에 다시 실행. 검증 자료·변경 전 백업: 현재 작업 공간 `work/exam-focused-writing/`
- 단계: 로컬 구현·UI Test·Build 완료. Commit·Push·Deploy 미실행, Production 미반영. 실제 학습자 정답률·Excel 실행 검증은 수행하지 않음
- 다음 범위: 다른 7개 초안도 연결 기출의 판단 근거와 대조해 필요성 검토. 이번 가이드 추가만으로 전체 기존 챕터의 축약이 완료된 것으로 해석하지 않음

---

## 2026-09-22 작성된 컴활 전체 초안의 기출 풀이 중심 정리 완료

- Owner 요청: 목표값 찾기에 적용한 기준을 현재 작성된 모든 챕터에 적용하고 중단된 작업 마무리
- 9월 19일 적용한 나머지 7개 초안·SVG 2개를 9월 22일 재확인하고 검증 완료. P2-20은 이전 완료본 보존. 전체 적용 범위는 컴활 초안 8개
- 중복 표·부연·다른 챕터와 겹치는 설명 축약. 단축키는 동작→키 목록, 계산은 중간값 순서로 정리. 이름 범위의 SUMPRODUCT 대응 곱·INDEX 상대 위치는 학습 그림 2장으로 전환
- 기출 정답 판단에 필요한 버전·대상·입력 조건 유지. 미배정 기초 3개도 후속 문제 이해에 필요한 개념을 보존하며 신규 배정하지 않음
- 문항별 유지·삭제 근거 및 검증: `docs/audits/2026-09-22-computer-literacy-all-drafts-review.md`
- Test: 초안 제목·기존 출처/편집 메모 보존, 계산 예시 독립 검산, 기출 DB·정답·배정·이미지 메타데이터·P2-20 변경 없음 확인. 실제 Excel 실행 결과로 해석하지 않음
- UI: 8개 챕터 390×844에서 가로 넘침 없음·학습 표 0개·h1 1개·편집 메모 비노출. 주 기출 버튼 14개 유지. 학습 SVG 4개 로딩, 대표 기출 2개의 팝업·정답 공개·닫기 및 이미지 확인. 임시 모바일 크기 해제
- Build: 2026-09-22 22:15 KST 성공, 256페이지·HTML 258개. 기존 500kB 경고 유지. SEO 257 URL 경고/오류 0, 공개 콘텐츠·deferred 검사 오류 0. 공개 산출물에 미리보기 경로·새 SVG 미포함
- 로컬 서버 재실행 및 전체 목차 연결 확인: `http://127.0.0.1:4321/admin/computer-literacy-learning/`
- 가이드 `CHAPTER_WRITING_GUIDE.md`는 이후 신규 집필과 개정에도 계속 적용. 고정 분량·같은 목차 형식을 채우는 작업 금지
