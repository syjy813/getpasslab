...
> **기록 범위**: 2026-06-15 (프로젝트 최초 대화) ~ 2026-07-14 (현재)
> **총 의사결정**: **76건** (채택 61 / 기각·폐기 15)
...
|------|------|
| ✅ **유효** | 현재도 그대로 적용 중 |
| ⚠️ **부분 유효** | 방향은 유지되나 세부가 바뀌었거나 재검토 필요 |
| 🔄 **대체됨** | 후속 결정으로 교체됨 (교체 결정 번호 명기) |
| ❌ **폐기** | 취소됨. 부활 조건 명기 |
| ⏳ **미검증** | 결정했으나 실행 여부 미확인 |
| 🔴 **코드와 불일치** | 결정은 있으나 실제 코드가 다름 |
...
>
> **가장 정합적인 해석**: 프로젝트는 **2026년 6월에 시작**했고(첫 대화 2026-06-15), 출시 목표는 **2026년 8월**이다. 산업안전기사 연 3회 시험 중 **3회차(통상 7~8월 필기)** 시즌 트래픽을 노리는 타이밍과 일치한다.
> 즉 **초기 문서의 "2025년"은 연도 오기일 가능성이 매우 높다.**
>
> **현재(2026-07-14)는 출시 목표까지 약 1개월 남은 시점이다.** SEO 인덱싱에 1~2주가 걸린다는 점을 감안하면 **매우 촉박하다.**
> ⚠️ **마일로에게 최우선으로 확인할 것.**
...
**현재 유효성**: ✅ **유효.** 실제로 CBT 해설집 PDF를 Google Drive에 올려 Cowork가 파싱하는 파이프라인으로 구현됐다.
...
**결정 이유**
- **기출문제 자체는 공공 저작물**(한국산업인력공단)이다
- 그러나 **타 사이트·유튜버의 해설과 표현은 그들의 창작물**이다
- 해설을 복제하면 저작권 침해이며, AdSense 승인에도 치명적

**고려했던 대안**

| 대안 | 기각 이유 |
|------|-----------|
| 유튜브 해설 영상 자막에서 해설까지 추출 | **저작권 침해** |
| 해설을 전부 자체 작성 | 1,675문항 규모에서 **1인 운영으로 불가능** |

**현재 유효성**: ✅ **유효.** 그 귀결로 **기출문제 모달에 해설이 없다.**
> **부활 조건**: 고빈출 문제 일부에 **자체 작성 해설**을 점진 추가하는 것은 저작권상 안전하며, 콘텐츠 차별화 효과가 크다. 검토 가치 있음.
...
**결정 내용**: `getpasslab.site` 구매 완료.

**현재 유효성**: ⏳ **미검증.** **도메인은 샀지만 아직 연결하지 않았다.**
- 현재 배포 주소: `syjy813.github.io/getpasslab/`
- 연결 시점: **AdSense 신청 직전**
- ⚠️ 연결 시 `astro.config.mjs`의 `base` 설정 변경 필수 (`withBase()`가 전체 링크를 자동 정정)
...
| GitHub Pages | ✅ | ✅ |
| KaTeX | ✅ | ✅ |
| **Tailwind** | 채택 | **🔴 설치되지 않음.** 순수 CSS + CSS 변수 |
| **Pagefind** | 채택 | **🔴 설치되지 않음. 검색 미구현** |
...
**결정 내용**
정적 사이트에서 `questions.json`을 클라이언트에 전달하므로 **정답 데이터가 노출된다.** 이를 **의도적으로 수용**한다.
...
**현재 유효성**: ✅ **유효.**
> 💡 **완화 요소 발견**: 실제 구현은 `questions.json` **전체를 내려주지 않는다.** 빌드 타임에 **챕터별로 필요한 문제만 HTML에 인라인**된다. 즉 한 챕터 페이지에는 그 챕터의 기출 N개만 존재한다. **1,675문항 전체가 한 번에 새지는 않는다.**
...
**라이선스 판단 (법적 리스크 회피)**
- TDS는 **앱인토스(미니앱) 파트너사에게만** 라이선스가 부여된다
- 자료의 권리는 지식재산권 포함 **전부 토스에 귀속**
- → **외부 일반 웹사이트가 TDS 컴포넌트를 가져다 쓰는 것은 라이선스 위반**
...
**현재 유효성**: ✅ **유효.**
> **이 판단이 없었다면 라이선스 분쟁 리스크가 있었다.** Claude가 웹 검색으로 라이선스 조건을 확인한 것이 유효했던 사례.
...
**현재 유효성**: ✅ **유효.** 🔴 단, **자체 호스팅이 아니라 jsDelivr CDN 로드**로 구현됨.
> 💡 **`dynamic-subset` 방식을 쓰고 있다** — 페이지에 실제 등장하는 글자만 다운로드. **한글 폰트 용량 문제의 정답.** 좋은 구현.
> 🔴 **잠재 버그**: `BaseLayout.astro`에 Pretendard 링크가 없다. `ChapterLayout.astro`에만 있다. **홈·과목 목차에서 폰트가 안 뜰 수 있다.** 확인 필요.
...
**현재 유효성**: 🔄 **대체됨.** 이후 기출 기반 재편성(D-35)을 거쳐 **253~254개**로 증가.
> ⚠️ **30개 증가 경위가 대화 기록에 명확히 남아 있지 않다.** 기출 재편성 과정에서 신규 챕터가 도출된 것으로 추정.
> **최신 검증값 (노션 CSV `_all` export 기준)**: **253행** / 작성완료 82 / 우선순위 **출시 필수 77 · 1차 160 · 2차 16**
...
**현재 유효성**: ✅ **유효.** 산출물이 `GetPassLab_Design_System.zip`으로 아카이브되어 Astro에 이식됨(D-57).
> **파생 규칙**: Claude Design 작업 시 **기존 GetPassLab 프로젝트를 열고 그 안에서 작업**한다. **새 프로젝트 생성 금지** (토큰·컴포넌트 재사용 불가해짐).
...
**현재 유효성**: ✅ **유효.** 이 3자 구조가 프로젝트의 실행 엔진이다.
...
**현재 유효성**: ✅ **유효 (완료됨).**
> ⚠️ **로컬 경로가 이후 변경된 이력 있음**: `C:\Users\정해준\...` → `C:\Users\Guns\Documents\GitHub\getpasslab`. Cowork 지시문의 경로를 최신값으로 통일할 것.
...
>
> **🚨 즉시 조치 필요**: ① 이미지 수집·삽입 또는 ② 해당 문제를 모달에서 노출 제외
...
**현재 유효성**: ✅ **유효 — 그리고 A안은 영원히 안 오게 됐다.**
> **D-68(노션 동결)로 인해 노션 API 연동 자체가 무의미해졌다.** B안이 결과적으로 최종안이 됐다.
...
**현재 유효성**: ✅ **유효.** 1,675문항이 단일 파일에 있다.
> ⚠️ **확장 시 재검토**: 자격증이 늘어 문항이 5,000개를 넘어가면 자격증별 분할 검토.
...
**현재 유효성**: ✅ **유효.**
> ✅ **이로써 `PROJECT_CONTEXT.md`의 "1,675 vs 1,680" 불확실성이 해소됐다.** 둘 다 맞고, 단계가 달랐다.
...
**현재 유효성 판단: ✅ 이 상태를 유지한다**

| 근거 | 설명 |
|------|------|
| 이미 253개 페이지 분량의 CSS가 동작한다 | 지금 갈아엎으면 회귀 버그 리스크만 크다 |
| **의존성 4개의 미덕** | 1인 운영에서 의존성은 유지보수 부채 |
...
**현재 유효성**: ✅ **유효.**
> ⚠️ **설계됐으나 미구현**: `/all` 과목 전체 보기 페이지, `/terms`, `/privacy`, `/admin/`, 검색 모달, 과목 탭 바
...
**현재 유효성**: ✅ **유효.**
> **최신 검증값**: **작성완료 82개** (출시 필수 74 + 마일로 직접 작성 초과분 8)
...
→ **253개 전체가 저장소에 존재**하되, `status: 미작성`은 **빌드에서 제외**한다.
...
**현재 유효성**: ⏳ **미검증. 🔴 그리고 현재 코드에 필터가 없다.**
> **확인된 실제 코드**:
> ```ts
> const countBy = (id: number) => chapters.filter(c => c.data.subject_id === id).length;
> //                                                    ^^^ status 필터 없음!
> ```
> 🚨 **이대로 스텁을 생성하면 "챕터 48개"라고 표시하고 클릭하면 12개만 나온다.**
> **이관 세션에서 반드시 4곳 전부에 필터를 추가해야 한다.**
>
> **검증**: `npm run build` 후 dist에 미작성 챕터 페이지가 생성되지 않았는지 확인. **새어나오면 작업 실패로 간주.**
...
> **🚨 노션 relation을 대량 병합하는 지금 시점에 특히 위험하다. 미매칭 ID 경고 출력 로직을 추가할 것.**
...
**현재 유효성**: ✅ **유효.**
> 💡 **일반화 가능한 원칙**: **"좋은 도구로 옮긴 뒤 정리하라. 나쁜 도구에서 정리한 뒤 옮기지 마라."**
...
**현재 유효성**: ✅ **유효.**
> **이런 도구 운용 규칙은 반드시 문서화해야 한다.** 안 그러면 6개월 뒤에 같은 실수를 반복한다.
...
**현재 유효성**: ⏳ **미검증.** Claude Design 지시문까지 작성 완료. 구현 여부 미확인.
...
## B.1 ✅ 유효 (핵심 불변식) — **절대 뒤집지 말 것**

| # | 결정 |
|---|------|
| D-01 | 포트폴리오 1순위 |
| D-03 | 문제·정답만 사용, 해설 자체 작성 |
| D-07 | 인접 자격증만 확장 (무관 카테고리 금지) |
| D-17 | 직접 구현 (노코드 기각) |
| **D-22** | **동결 키 3종** |
...
## B.2 🔴 결정은 있으나 코드가 다르다 — **즉시 확인**

| # | 결정 | 실제 |
|---|------|------|
| D-18 | Tailwind 채택 | **미설치.** 순수 CSS (→ D-58) |
| D-18 | Pagefind 검색 | **미설치. 검색 미구현** |
| D-48 | 헤더 검색바 | **GNB에 검색 아이콘 없음** |
| D-28 | Pretendard 자체 호스팅 | **jsDelivr CDN.** 게다가 **BaseLayout에 링크 없음** (버그 가능성) |
| D-70 | status 빌드 필터 4곳 | **필터 없음.** 스텁 생성 시 카운트 오류 발생 |

## B.3 ⚠️ 미결정 — **작업 전 마일로에게 확인**

| # | 항목 | 결정 시한 |
|---|------|-----------|
| 1 | **출시 목표 시점** (2026년 8월?) | **즉시** |
...
## B.4 ❌ 폐기된 결정 — **부활 조건 있는 것만**

| # | 폐기안 | 부활 조건 |
|---|--------|-----------|
| D-04 | YouTube 자막 파싱 | 없음 |
| D-08 | 확장 순서 1차안 (위험물기능사) | 없음 (D-09가 대체) |
| D-37 | 챕터 최소 1,500자 | **없음. 자연 분량 원칙이 대체** |
| D-25 | 노션 = 작업 도구 | **없음. 이중 원본 금지가 대체** |
