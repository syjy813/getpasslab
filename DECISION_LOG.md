...
> **기존 결정 ID**: `D-01`~`D-76` 보존 / 현행 운영 결정 `D-77`~`D-82` 추가
...
## D-82. 기출문제 데이터는 자격증별 파일로 물리 분리

**일시**: 2026-08-04

**Decision status**: **ACTIVE**
**Decision owner**: **Owner**
**Supersedes**: D-47의 "모든 자격증을 하나의 `questions.json`에 저장" 범위

**결정 내용**
- 기출문제 정본은 `src/data/questions/{cert_id}.json`에 자격증별로 분리한다.
- 빌드에서는 전용 로더가 이 파일들을 하나의 Astro content collection으로 읽는다.
- 공개 여부는 데이터 파일 존재가 아니라 챕터 frontmatter의 `questions` 관계로만 결정한다.
- 신규 자격증 데이터를 내재화해도 관계를 연결하기 전에는 공개 페이지에 문항·정답을 렌더하지 않는다.
- `question_id`, `subject_id`, 기존 챕터 관계와 URL 규칙은 변경하지 않는다.
- 파일명과 데이터의 `cert_id`가 다르거나 같은 자격증·시험 안에서 ID가 중복되면 빌드를 실패시킨다.

**초기 저장 게이트 구현 범위 (2026-08-04)**
- `industrial-safety.json`: 기존 1,680문항을 바이트 동일하게 이동
- `energy-management.json`: 2010-01-31~2016-07-10, 27회×60문항 내재화
- 이 초기 게이트에서는 에너지관리기능사 챕터 관계 0건 유지

**후속 로컬 구현 상태 (2026-08-05)**
...
flowchart TD
    ...
    D47["D-47<br/>기출 단일 파일"] -->|superseded| D82["D-82<br/>자격증별 기출 파일"]
...
| **D-82** | **기출문제 자격증별 파일 분리·관계 전 공개 금지** |
...
| D-47 | 모든 자격증 기출을 하나의 JSON에 저장 | **SUPERSEDED by D-82.** 자격증별 파일로 분리 |
