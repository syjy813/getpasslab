...
> 챕터 내용의 포함·삭제 기준은 [전체 챕터 작성 가이드](CHAPTER_WRITING_GUIDE.md)를 따른다. 기출 풀이에 불필요한 내용은 넣지 않으며, 페이지 구성 틀을 채우기 위한 본문·소제목은 만들지 않는다.

| 규칙 | 근거 |
|------|------|
| **분량을 부풀리지 않는다** | 기출 풀이에 필요한 분량만 작성. 고정 글자 수·목차를 채우기 위한 부연 금지 |
| **h1은 페이지당 1개** | 챕터명이 h1 |
